...
void InputGlyphDemoTool::reload_glyphs() {
	...
	InputType input_type = (InputType)input_type_idx;
	...
	for (int theme_i = 0; theme_i < THEME_COUNT; theme_i++) {
		...
		// Now do the rest
		for (int i = 4; i < InputOrigin::INPUT_ORIGIN_COUNT; i++) {
			GlyphDebugInfo info;
			info.origin = (InputOrigin)i;
...
void InputGlyphDemoTool::draw_glyphs(int p_theme_i) {
	for (int i = 0; i < GLYPH_COUNT; i++) {
		...
		if (ImGui::IsItemHovered()) {
			ImGui::BeginTooltip();
			ImGui::TextUnformatted(InputGlyphTables::_debug_input_origin_names[glyph_infos[p_theme_i][i].origin]);
...
void InputGlyphDemoTool::draw_ui() {
	...
	static_assert(std::size(input_type_names) == InputType::INPUT_TYPE_MAX);
	...
	for (int i = 0; i < connected.size(); i++) {
		...
		InputType it = InputGlyphsSingleton::get_singleton()->glyph_source->identify_joy(number);
