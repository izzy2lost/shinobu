...
bool check_edge_removal(const IntPoint pt1, const IntPoint pt2,
		const IntPoint pt3) {
	Vector3 p1 = Vector3(pt1.X, pt1.Z, pt1.Y) / SCALE_FACTOR;
	Vector3 p2 = Vector3(pt2.X, pt2.Z, pt2.Y) / SCALE_FACTOR;
	Vector3 p3 = Vector3(pt3.X, pt3.Z, pt3.Y) / SCALE_FACTOR;
	Vector3 a = p2 - p1;
	Vector3 b = p3 - p1;
	Vector3 segment[2]{
		p1,
	...
	Vector3 closest = Geometry3D::get_closest_point_to_segment(p2, segment);
	if (p3.distance_to(closest) < CMP_EPSILON) {
		//return false;
	}
	if (!Math::is_equal_approx(p1.y, p2.y) || !Math::is_equal_approx(p2.y, p3.y) || pt1.Z != pt2.Z || pt2.Z != pt3.Z) {
		return false;
	}
	return true;
}
	...
	for (;;) {
		...
		if (E->Prev == E->Next)
			break; //only two vertices
				...
				((!m_PreserveCollinear && check_edge_removal(E->Prev->Curr, E->Curr, E->Next->Curr)) ||
	...
	for (;;) {
		...
		if ((pp->Pt == pp->Next->Pt) || (pp->Pt == pp->Prev->Pt) ||
				(SlopesEqual(pp->Prev->Pt, pp->Pt, pp->Next->Pt, m_UseFullRange) &&
						((!preserveCol && check_edge_removal(pp->Prev->Pt, pp->Pt, pp->Next->Pt)) || !Pt2IsBetweenPt1AndPt3(pp->Prev->Pt, pp->Pt, pp->Next->Pt)))) {
			lastOK = 0;
			OutPt *tmp = pp;
